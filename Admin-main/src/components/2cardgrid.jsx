...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (results[5].status === "fulfilled") {
          ...
          setRevenueData(Array.isArray(data) ? data : data?.results || []);
        } else {
          setRevenueData([]);
        }
        ...
        if (results[6].status === "fulfilled") {
          ...
          setSalesData(Array.isArray(data) ? data : data?.results || []);
        } else {
          setSalesData([]);
        }
      } catch (err) {
  ...
  const filteredClaims = useMemo(
   ...
   const loanApplicationsCount = loans.length;
   const pendingReturnsCount  = claims.filter(
