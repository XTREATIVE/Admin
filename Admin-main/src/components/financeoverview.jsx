...
const FinanceOverview = () => {
  ...
  const fetchPayouts = async () => {
    try {
      ...
      const payoutsArray = Array.isArray(payouts) ? payouts : (payouts?.results || []);
      const filteredPayouts = payoutsArray.filter((payout) => {
      ...
      console.log('Payouts processed - settled:', settled, 'pending:', pending);
  ...
  const fetchInventoryValue = async () => {
    try {
      ...
      const stockArray = Array.isArray(stockData) ? stockData : (stockData?.results || []);
      
      // Debug: log the first item to see the structure
      if (stockArray.length > 0) {
        console.log('First stock item:', stockArray[0]);
      }
      
      const totalValue = stockArray.reduce((sum, item) => {
        // Try multiple possible field names for price and quantity
        const price = parseFloat(item.price || item.selling_price || item.unit_price || item.product_price || 0);
        const quantity = parseInt(item.quantity || item.stock_quantity || item.stock || item.available_stock || 0);
        const itemValue = price * quantity;
        if (itemValue > 0) {
          console.log(`Item: ${item.product_name || item.name || 'Unknown'}, Price: ${price}, Qty: ${quantity}, Value: ${itemValue}`);
        }
      ...
      console.error('Error fetching inventory value:', error.message || error);
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('Listing data response:', listingData);
          const listingArray = Array.isArray(listingData) ? listingData : (listingData?.results || []);
          const totalValue = listingArray.reduce((sum, item) => {
            const price = parseFloat(item.price || item.selling_price || 0);
            const quantity = parseInt(item.stock_quantity || item.quantity || 0);
            return sum + (price * quantity);
          }, 0);
          console.log('Inventory value from listing:', totalValue);
          setInventoryValue(totalValue);
        }
      } catch (fallbackError) {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      console.log('FinanceOverview: Starting data fetch...');
      console.log('Context orders:', contextOrders?.length || 0);
      console.log('Context claims:', contextClaims?.length || 0);
      console.log('Range:', range, 'Custom date:', customDate);
      setDataLoading(true);
      await Promise.all([fetchPayouts(), fetchInventoryValue(), generateMonthlyTrends()]);
      setDataLoading(false);
      console.log('FinanceOverview: Data fetch complete');
    };
