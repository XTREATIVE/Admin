...
const LoginScreen = () => {
  ...
  const { fetchClaims } = useContext(ClaimsContext);
  ...
  const handleLogin = async (values, { setSubmitting }) => {
    try {
      ...
      if (response.ok && data.access) {
        ...
        // Fetch orders and claims immediately after login
        await fetchOrders();
        await fetchClaims();
        ...
        setLoginError(data.message || data.detail || "Invalid credentials");
  ...
  return (
    <div className="login-screen font-poppins">
      <style>{`
        ...
        .login-card { width: 100%; max-width: 450px; background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 40px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); position: relative; }
        .login-card-title { text-align: center; font-size: 2rem; font-weight: bold; margin-bottom: 32px; color: #280300; }
        .login-form { display: flex; flex-direction: column; }
        .form-group { position: relative; margin-bottom: 24px; }
        .login-input { width: 100%; padding: 12px 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; outline: none; transition: border-color 0.2s; }
        .login-input:focus { border-color: #6B46C1; }
        .login-label { position: absolute; left: 12px; top: 12px; font-size: 14px; color: #999; background: transparent; padding: 0 4px; transition: all 0.2s; pointer-events: none; }
        ...
        .login-button { width: 100%; background-color: #280300; color: #fff; padding: 12px; border: none; border-radius: 4px; font-size: 16px; cursor: pointer; transition: background-color 0.3s; }
        ...
        .error-message { background-color: #ffe6e6; color: #d9534f; }
        .success-message { background-color: #e6ffe6; color: #28a745; }
