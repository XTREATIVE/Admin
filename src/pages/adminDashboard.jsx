...
import {
  ...
  getReturnsList, // API to fetch returns/claims list
} from "../api.js";
...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (results[5].status === 'fulfilled') {
          const returnsData = results[5].value;
          const returns = Array.isArray(returnsData) ? returnsData : (returnsData?.results || []);
          setReturns(returns);
        } else {
          console.warn("Failed to fetch returns:", results[5].reason.message);
          setReturns([]);
        }
  ...
  const filteredClaims = useMemo(
    () => (Array.isArray(returns) ? returns : []).filter((c) => inRange(c.created_at)),
    [returns, range, customDate]
  );
