...
export const LoansProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch loans
        const loansRes = await fetch('https://xtreativeapi.onrender.com/loan_app/loans/list/', {
        ...
        // Fetch vendors
        const vendorsRes = await fetch('https://xtreativeapi.onrender.com/vendors/list/', {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (vendorsRes.ok) {
  ...
  const value = {
    ...
    rejectLoan: async (id, reason) => {
      try {
        const token = localStorage.getItem('authToken');
        const res = await fetch(`https://xtreativeapi.onrender.com/loan_app/${id}/reject/`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
