...
export const LoansProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch loans
        const loansRes = await fetch('https://api-xtreative-nwf7.onrender.com/loan_app/loans/list/', {
        ...
        // Fetch vendors
        const vendorsRes = await fetch('https://api-xtreative-nwf7.onrender.com/vendors/list/', {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (vendorsRes.ok) {
  ...
  const value = {
    ...
    approveLoan: async (id) => {
      try {
        const token = localStorage.getItem('authToken');
        const res = await fetch(`https://api-xtreative-nwf7.onrender.com/loan_app/${id}/approve/`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
        });
        if (res.ok) {
    ...
    rejectLoan: async (id, reason) => {
      try {
        const token = localStorage.getItem('authToken');
        const res = await fetch(`https://api-xtreative-nwf7.onrender.com/loan_app/${id}/reject/`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
